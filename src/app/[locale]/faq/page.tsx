...
export default function FAQPage() {
  ...
  const spanishFAQs: FAQItem[] = [
    ...
    },
    {
      ...
      answer:
        'Usamos blockchain y herramientas modernas para garantizar transparencia, seguridad y bajas comisiones.',
    },
    {
      id: 'es-5',
  ...
  const englishFAQs: FAQItem[] = [
    ...
    },
    {
      ...
      answer:
        'Browse the catalog, purchase, and track deliveries in real time with verified reputation.',
    },
    {
      id: 'en-4',
      question: 'What technology do you use?',
      answer:
        'We use blockchain and modern tooling to ensure transparency, security, and low fees.',
    },
    {
      id: 'en-5',
  ...
  const FAQSection = ({ items, title }: { items: FAQItem[]; title?: string }) => (
    <div className="space-y-4">
      {title && (
        <h2 className="text-xl md:text-2xl font-semibold text-gray-800 mb-4 md:mb-6 mx-4 md:mx-6">
          {title}
        </h2>
      )}
      <div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-green-100 py-8 md:py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-gray-600 max-w-3xl mx-auto">{t('subtitle')}</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
        <div className="space-y-8 md:space-y-12 bg-white rounded-2xl border border-green-200 shadow-sm overflow-hidden p-4 md:p-6">
          <FAQSection items={locale === 'es' ? spanishFAQs : englishFAQs} />
        </div>
      </div>
    </div>
  );
