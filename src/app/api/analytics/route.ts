...
function convertFunnelsToCSV(funnels: unknown): string {
  ...
  const headers = ['source', 'pageViews', 'formStarts', 'formSubmissions', 'successfulSignups', 'conversionRate'];
...
function convertArrayToCSV(data: unknown, headers: string[]): string {
  ...
  const rows = data.map((item: Record<string, unknown>) => headers.map(header => item[header] || ''));
  return [headers.join(','), ...rows.map(row => row.join(','))].join('\n');
}
...
function convertSummaryToCSV(data: unknown): string {
  ...
  const rows = [
    ...
    ['Form Submissions', summary.total.formSubmissions, summary.today.formSubmissions, summary.week.formSubmissions],
    ['Successful Signups', summary.total.successfulSignups, summary.today.successfulSignups, summary.week.successfulSignups],
  ];
