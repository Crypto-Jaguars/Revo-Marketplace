...
const LoginModal: React.FC<LoginModalProps> = ({ onClose, t }) => {
  ...
  const handleRedirectToAuth = () => {
    ...
    router.push(`/${language}/auth`); // Navigate to the new auth page
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 relative max-w-lg w-full">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            onClick={handleRedirectToAuth}
            className="border border-green-600 text-green-600 py-2 rounded-full shadow-md hover:bg-green-50 transition-colors text-sm"
          >
